...
export default function BroodingBatchDetailScreen({ batchId, lossRecords = [], vaccinationSchedule = [], vaccineCompletions = {}, readyDay = 42, onBack, onSaveLoss, onMarkVaccineCompleted }) {
  ...
  const vaccineTiming = nextVaccine ? previewDay > nextVaccine.day ? `${previewDay - nextVaccine.day} days overdue` : previewDay === nextVaccine.day ? 'Due today' : `${nextVaccine.day - previewDay} days remaining` : '';
  const firstWeekDate = dateAfterDays(batch.hatchDate, 7);
  const nextAction = readyForGrowing
    ? { icon: 'arrow-right-circle-outline', title: 'Ready for Growing', detail: `Due ${readyDate} - farmer confirmation required`, button: 'Move to Growing', onPress: () => setShowGrowing(true) }
    : vaccineNeedsAction
      ? { icon: 'needle', title: `${nextVaccine.name} Vaccine Due`, detail: `Due ${dateAfterDays(batch.hatchDate, nextVaccine.day)} - ${vaccineTiming}`, button: 'Record Completed', onPress: () => onMarkVaccineCompleted(nextVaccine.id) }
      : previewDay <= 1
        ? { icon: 'home-check-outline', title: 'Confirm Brooder Setup', detail: `Due ${batch.hatchDate} - confirm the batch is settled`, button: 'Complete', onPress: () => Alert.alert('Brooder setup confirmed') }
        : previewDay < 7
          ? { icon: 'clipboard-check-outline', title: 'First Week Check', detail: `Due ${firstWeekDate} - ${7 - previewDay} day${7 - previewDay === 1 ? '' : 's'} remaining`, button: 'Start Check', onPress: () => Alert.alert('First Week Check', batch.id) }
          : { icon: 'chart-timeline-variant', title: 'Growing Transition Approaching', detail: `Due ${readyDate} - ${Math.max(0, readyDay - previewDay)} days remaining`, button: 'View Program', onPress: () => Alert.alert('Brooding program', `Day ${previewDay} of the ${readyDay}-day brooding cycle.`) };
  const nextActionStatus = readyForGrowing ? 'Ready' : vaccineNeedsAction ? nextVaccineStatus : previewDay === 0 ? 'Due Today' : previewDay === 1 ? 'Overdue' : 'Upcoming';
  const nextActionTone = nextActionStatus === 'Overdue' ? '#ef7568' : nextActionStatus === 'Ready' ? '#ffba56' : ORANGE;
...
const styles = StyleSheet.create({
  ...
  originStrip: { minHeight: 58, marginTop: 8, borderRadius: 7, borderWidth: 1, borderColor: '#1d2d33', backgroundColor: '#081216', flexDirection: 'row', alignItems: 'center' }, originItem: { flex: 1, minWidth: 0, alignItems: 'center', paddingHorizontal: 5 }, originDivider: { width: 1, height: 30, backgroundColor: '#213139' }, snapshotGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 8 }, snapshotItem: { flex: 1, minWidth: 0, height: 94, borderRadius: 7, borderWidth: 1, borderColor: '#23343b', backgroundColor: '#091317', paddingHorizontal: 7, alignItems: 'center', justifyContent: 'center' }, snapshotItemCompact: { flexBasis: '48%', height: 86 }, snapshotValue: { width: '100%', marginTop: 7, color: '#eef2f3', fontSize: 13, fontWeight: '800', textAlign: 'center' }, snapshotLabel: { marginTop: 4, color: '#718086', fontSize: 7, textAlign: 'center' }, actionFocus: { minHeight: 98, borderRadius: 7, borderWidth: 1, borderColor: '#74410e', backgroundColor: 'rgba(255,121,0,0.06)', padding: 11, flexDirection: 'row', alignItems: 'center', gap: 10 }, actionFocusIcon: { width: 44, height: 44, borderRadius: 22, backgroundColor: 'rgba(255,121,0,0.1)', alignItems: 'center', justifyContent: 'center' }, actionFocusCopy: { flex: 1, minWidth: 0 }, actionFocusTitle: { color: '#f0f3f4', fontSize: 11, fontWeight: '800' }, actionFocusDetail: { marginTop: 4, color: '#8d9a9e', fontSize: 8, lineHeight: 12 }, actionFocusControls: { width: 96, alignItems: 'stretch', gap: 6 }, actionStatus: { minHeight: 21, maxWidth: 96, paddingHorizontal: 7, borderRadius: 11, borderWidth: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 4 }, actionStatusDot: { width: 5, height: 5, borderRadius: 3 }, actionStatusText: { flexShrink: 1, fontSize: 7, fontWeight: '800' }, actionFocusButton: { width: '100%', height: 38, borderRadius: 6, backgroundColor: ORANGE, paddingHorizontal: 7, alignItems: 'center', justifyContent: 'center' }, actionFocusButtonText: { color: '#fff', fontSize: 8, fontWeight: '800', textAlign: 'center' },
