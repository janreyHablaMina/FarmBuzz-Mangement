...
export default function BroodingBatchDetailScreen({ batchId, batchOverride, lossRecords = [], vaccinationSchedule = [], vaccineCompletions = {}, readyDay = 42, onBack, onSaveLoss, onMarkVaccineCompleted }) {
  const { width } = useWindowDimensions();
  const compact = width < 480;
  const [showLoss, setShowLoss] = useState(false);
  const batch = useMemo(() => batchOverride || BROODING_BATCHES.find((item) => item.id === batchId) || BROODING_BATCHES[0], [batchId, batchOverride]);
