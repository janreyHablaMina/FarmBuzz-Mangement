...
export default function RecordHatchScreen({ batchId, activeEggCount, initialResult, batchOverride, sourcesOverride, onBack, onSave }) {
  ...
  const save = () => Alert.alert('Save hatch result?', `${hatched} chicks hatched from ${hatchEggs} eggs.`, [{ text: 'Cancel', style: 'cancel' }, { text: 'Save Hatch', onPress: () => onSave({ saved: true, batchId, hatchDate, eggsAtHatch: hatchEggs, hatched, unhatched, hatchRate, sources: sources.map((source) => ({ ...source, hatched: counts[source.groupName], marking: markings[source.groupName].trim() })) }) }]);
      ...
      <ScrollView keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false} contentContainerStyle={styles.pageWrap}>
        <View style={styles.page}>
          ...
          <View style={styles.content}>
            ...
            <View style={styles.sourceList}>{sources.map((source, index) => <View key={source.groupName} style={[styles.sourceRow, index > 0 && styles.divider]}><View style={styles.sourceTop}><View><Text style={styles.sourceName}>{source.groupName}</Text><Text style={styles.sourceCross}>{source.cross}</Text></View><Text style={styles.sourceEggs}>{source.eggs} eggs</Text></View><View style={styles.sourceControls}><Text style={styles.controlLabel}>Chicks Hatched</Text><View style={styles.stepper}><Pressable onPress={() => changeCount(source, -1)} style={styles.stepButton}><Ionicons name="remove" size={17} color="#abb6b9" /></Pressable><TextInput value={String(counts[source.groupName])} onChangeText={(value) => enterCount(source, value)} keyboardType="number-pad" inputMode="numeric" selectTextOnFocus maxLength={3} style={styles.stepValue} /><Pressable onPress={() => changeCount(source, 1)} style={styles.stepButton}><Ionicons name="add" size={17} color="#fff" /></Pressable></View></View><View style={styles.markingField}><MaterialCommunityIcons name="tag-outline" size={17} color={ORANGE} /><TextInput value={markings[source.groupName]} onChangeText={(value) => setMarkings((current) => ({ ...current, [source.groupName]: value }))} placeholder="Optional chick marking" placeholderTextColor="#68777c" style={styles.markingInput} /></View></View>)}</View>
