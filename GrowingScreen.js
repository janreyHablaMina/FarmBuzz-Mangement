...
export const GROWING_BATCHES = [
  { id: 'GR-024', birds: 38, age: '8 Weeks', ageDays: 56, location: 'Grower House 1', status: 'Growing', nextAction: '' },
  { id: 'GR-018', birds: 42, age: 'Month 3', ageDays: 90, location: 'Grower Area 2', status: 'Growing', nextAction: 'Health task due in 2 days' },
  { id: 'GR-015', birds: 27, age: 'Month 2', ageDays: 68, location: 'Grower House 2', status: 'Needs Attention', nextAction: 'Health review overdue' },
  { id: 'GR-011', birds: 31, age: 'Month 4', ageDays: 120, location: 'Grower Area 1', status: 'Ready for Ranging', nextAction: 'Move to Ranging' },
];
...
function BatchCard({ batch, onPress }) {
  ...
  const progress = Math.min(100, Math.round((batch.ageDays / 120) * 100));
  return (
...
export default function GrowingScreen({ onBack }) {
  const { width } = useWindowDimensions();
  const compact = width < 480;
  const [query, setQuery] = useState('');
  const batches = useMemo(() => {
    const search = query.trim().toLowerCase();
    if (!search) return GROWING_BATCHES;
    return GROWING_BATCHES.filter((batch) => `${batch.id} ${batch.location} ${batch.status} ${batch.age} ${batch.nextAction}`.toLowerCase().includes(search));
  }, [query]);
  const summary = [
    ...
    { icon: 'arrow-right-circle-outline', value: String(GROWING_BATCHES.filter((batch) => batch.status === 'Ready for Ranging').length), label: 'Ready for Ranging', detail: 'around month 4' },
  ];
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.pageWrap}>
        <View style={styles.page}>
          ...
          <View style={[styles.content, compact && styles.contentCompact]}>
            ...
            <View style={styles.batchList}>{batches.map((batch) => <BatchCard key={batch.id} batch={batch} onPress={() => Alert.alert(batch.id, `${batch.birds} birds in ${batch.location}`)} />)}{!batches.length && <Text style={styles.empty}>No growing batches found</Text>}</View>
          </View>
        </View>
      </ScrollView>
    </View>
