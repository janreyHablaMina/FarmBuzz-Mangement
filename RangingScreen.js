...
export const RANGING_BATCHES = [
  ...
  { id: 'RG-018', birds: 31, startingBirds: 32, hatchDate: '2026-03-18T12:00:00', rangingStartDate: '2026-07-18T12:00:00', location: 'North Range', status: 'Ready for Cording', nextAction: 'Move to Cording', growingBatch: 'GR-018', sources: [{ name: 'Main Breeders', cross: 'Sweater x Kelso', birds: 18, marking: 'Red' }, { name: 'Group C', cross: 'Roundhead x Hatch', birds: 13, marking: 'Yellow' }] },
];
...
export function buildRangingLocations(batches, lossesByBatch = {}, locationsByBatch = {}, selectionsByLocation = {}, readyDay = SELECTION_AGE_DAYS) {
  ...
  return [...grouped.values()].map((group) => {
    ...
    return { ...group, birds, status: ready ? 'Ready for Cording' : 'Active', ageRange: minAge === maxAge ? monthAge(minAge) : `${monthAge(minAge)} - ${monthAge(maxAge)}`, lastSelectionDate: selection?.date || '', selection };
...
export default function RangingScreen({ farm, batches = RANGING_BATCHES, lossesByBatch = {}, locationsByBatch = {}, selectionsByLocation = {}, taskSchedule = [], taskCompletionsByBatch = {}, readyDay = SELECTION_AGE_DAYS, nextCordingBirdNumber = 101, onBack, onOpenSettings, onOpenBatch, onMarkTaskDone, onMoveToCording }) {
  ...
  const openCordingEntry = (group, taskId) => { setBloodline(''); setBloodlineDropdownOpen(false); setTpNumber(''); setCordingEntry({ group, taskId }); };
  const saveCordingEntry = () => {
    ...
    onMoveToCording({ location: cordingEntry.group.location, taskId: cordingEntry.taskId, farmBuzzId, bloodline, tpNumber: tpNumber.trim() });
