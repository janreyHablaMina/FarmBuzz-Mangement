...
const STAGES = [
  ...
  { day: 10, label: 'Candling', icon: 'flashlight', image: DEVELOPING_EGG_IMAGE },
  { day: 14, label: 'Developing', icon: 'egg', image: CANDLING_EGG_IMAGE },
  { day: 18, label: 'Lockdown', icon: 'progress-clock', image: HATCHING_EGG_IMAGE },
...
export default function FamilyBatchDetailScreen({ batchId, candlingResults, hatchResults, onBack, onOpenCandling, onOpenHatch, onSaveCandling }) {
  ...
  const batch = useMemo(() => ({
    ...
    eggs: '100 eggs',
    eggCount: 100,
    incubator: 'North House Batch',
    candlingDueDay: 10,
  }), [batchId]);
  const [previewDay, setPreviewDay] = useState(batch.dayNumber);
  const [localCandlingResults, setLocalCandlingResults] = useState(null);
  const [candlingModalVisible, setCandlingModalVisible] = useState(false);
  const [fertileEggs, setFertileEggs] = useState('');
  const savedResults = localCandlingResults ?? candlingResults ?? {};
  ...
  const fertilityRate = savedResults.fertilityRate ?? 0;
  ...
  const showHatchAction = hasCandled;
  const hatchRecordingAvailable = previewDay >= 21;
  const previewActionDisabled = hatchComplete || (showHatchAction ? !hatchRecordingAvailable : !previewCandlingAvailable);
  const nextMilestone = previewDay >= 18
      ? { value: 'Lockdown', label: previewDay >= 21 ? 'Record Hatch' : 'Lockdown Period' }
      : previewDay >= 14
  ...
  const openCandlingModal = () => {
    setFertileEggs(candlingComplete ? String(developingCount) : '');
    setCandlingModalVisible(true);
  };

  const saveCandling = () => {
    const fertile = Number(fertileEggs);
    if (!fertileEggs || !Number.isInteger(fertile) || fertile < 0 || fertile > eggsSet) {
      Alert.alert('Check fertile eggs', `Enter a whole number from 0 to ${eggsSet}.`);
      return;
    }
    const result = {
      saved: true,
      developing: fertile,
      rejected: eggsSet - fertile,
      recheck: 0,
      fertilityRate: Math.round((fertile / eggsSet) * 1000) / 10,
    };
    setLocalCandlingResults(result);
    onSaveCandling?.(result);
    setCandlingModalVisible(false);
  };
  ...
  return (
    <View style={styles.screen}>
        ...
        <View style={styles.page}>
          ...
          <View style={[styles.content, compact && styles.contentCompact]}>
            ...
            <View style={styles.metricsCard}>
              ...
              <Metric icon="percent-outline" value={candlingComplete ? `${fertilityRate}%` : '--'} label="Fertility Rate" />
            ...
            <View style={styles.actionCard}>
              ...
              <View style={styles.actionCopy}>
                <Text style={styles.actionTitle}>{hatchComplete ? 'Hatch Recorded' : showHatchAction ? 'Record Hatch' : previewCandlingAvailable ? 'Candling Due' : 'Incubating'}</Text>
                <Text style={styles.actionDetail}>
                  {hatchComplete
                    ? `${hatchResults.hatched} chicks - ${hatchResults.unhatched} unhatched - ${hatchResults.hatchRate}% hatch rate`
                    : showHatchAction
                      ? hatchRecordingAvailable
                        ? `${activeEggCount} eggs ready for hatch recording`
                        : `Available on Day 21 - ${activeEggCount} eggs continuing incubation`
              ...
              <Pressable
                ...
                onPress={showHatchAction ? onOpenHatch : openCandlingModal}
                style={({ pressed }) => [
                  styles.actionButton,
      ...
      <Modal visible={candlingModalVisible} transparent animationType="fade" onRequestClose={() => setCandlingModalVisible(false)}>
        <View style={styles.candlingModalOverlay}>
          <Pressable accessibilityLabel="Close candling modal" onPress={() => setCandlingModalVisible(false)} style={StyleSheet.absoluteFill} />
          <View style={styles.candlingModalCard}>
            <View style={styles.candlingModalHeader}>
              <View>
                <Text style={styles.candlingModalEyebrow}>DAY 10 CHECK</Text>
                <Text style={styles.candlingModalTitle}>Record Candling</Text>
              </View>
              <Pressable accessibilityLabel="Close" onPress={() => setCandlingModalVisible(false)} style={({ pressed }) => [styles.candlingModalClose, pressed && styles.pressed]}>
                <Ionicons name="close" size={20} color="#dfe5e7" />
              </Pressable>
            </View>

            <Text style={styles.candlingModalDescription}>Enter how many eggs showed development during candling.</Text>

            <View style={styles.candlingTotalRow}>
              <MaterialCommunityIcons name="egg-outline" size={20} color={ORANGE} />
              <Text style={styles.candlingTotalLabel}>Eggs set</Text>
              <Text style={styles.candlingTotalValue}>{eggsSet}</Text>
            </View>

            <Text style={styles.candlingInputLabel}>FERTILE EGGS</Text>
            <View style={styles.candlingInputShell}>
              <MaterialCommunityIcons name="egg-easter" size={20} color={ORANGE} />
              <TextInput
                autoFocus
                value={fertileEggs}
                onChangeText={(value) => setFertileEggs(value.replace(/[^0-9]/g, ''))}
                placeholder="Enter fertile egg count"
                placeholderTextColor="#66777d"
                selectionColor={ORANGE}
                keyboardType="number-pad"
                inputMode="numeric"
                maxLength={3}
                style={styles.candlingInput}
              />
            </View>

            <View style={styles.candlingCalculationRow}>
              <View style={styles.candlingCalculationItem}>
                <Text style={styles.candlingCalculationLabel}>Infertile</Text>
                <Text style={styles.candlingCalculationValue}>{fertileEggs ? Math.max(0, eggsSet - Number(fertileEggs || 0)) : '--'}</Text>
              </View>
              <View style={styles.candlingCalculationDivider} />
              <View style={styles.candlingCalculationItem}>
                <Text style={styles.candlingCalculationLabel}>Fertility Rate</Text>
                <Text style={[styles.candlingCalculationValue, styles.candlingCalculationAccent]}>{fertileEggs ? `${Math.min(100, Math.round((Number(fertileEggs) / eggsSet) * 1000) / 10)}%` : '--'}</Text>
              </View>
            </View>

            <View style={styles.candlingModalActions}>
              <Pressable onPress={() => setCandlingModalVisible(false)} style={({ pressed }) => [styles.candlingCancelButton, pressed && styles.pressed]}>
                <Text style={styles.candlingCancelText}>Cancel</Text>
              </Pressable>
              <Pressable onPress={saveCandling} style={({ pressed }) => [styles.candlingSaveButton, pressed && styles.pressed]}>
                <Ionicons name="checkmark" size={18} color="#ffffff" />
                <Text style={styles.candlingSaveText}>Save Result</Text>
              </Pressable>
            </View>
          </View>
        </View>
...
const styles = StyleSheet.create({
  ...
  candlingModalOverlay: {
    flex: 1, backgroundColor: 'rgba(0, 4, 6, 0.82)', paddingHorizontal: 16,
    alignItems: 'center', justifyContent: 'center',
  },
  candlingModalCard: {
    width: '100%', maxWidth: 430, borderRadius: 8, borderWidth: 1, borderColor: '#2a3b42',
    backgroundColor: '#091317', padding: 16,
  },
  candlingModalHeader: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: 12 },
  candlingModalEyebrow: { color: ORANGE, fontSize: 9, fontWeight: '800', letterSpacing: 0 },
  candlingModalTitle: { marginTop: 4, color: '#f0f3f4', fontSize: 20, fontWeight: '800', letterSpacing: 0 },
  candlingModalClose: {
    width: 36, height: 36, borderRadius: 18, borderWidth: 1, borderColor: '#304047',
    backgroundColor: '#101c21', alignItems: 'center', justifyContent: 'center',
  },
  candlingModalDescription: { marginTop: 10, color: '#8b989c', fontSize: 11, lineHeight: 17 },
  candlingTotalRow: {
    marginTop: 16, height: 46, borderRadius: 7, backgroundColor: '#0d1a1f',
    paddingHorizontal: 12, flexDirection: 'row', alignItems: 'center', gap: 8,
  },
  candlingTotalLabel: { flex: 1, color: '#a7b1b4', fontSize: 11 },
  candlingTotalValue: { color: '#ffffff', fontSize: 16, fontWeight: '800' },
  candlingInputLabel: { marginTop: 16, marginBottom: 6, color: '#849196', fontSize: 9, fontWeight: '800' },
  candlingInputShell: {
    height: 50, borderRadius: 7, borderWidth: 1, borderColor: 'rgba(255, 122, 0, 0.55)',
    backgroundColor: '#0b161a', paddingHorizontal: 12, flexDirection: 'row', alignItems: 'center', gap: 9,
  },
  candlingInput: { flex: 1, height: 48, paddingVertical: 0, color: '#ffffff', fontSize: 14, outlineStyle: 'none' },
  candlingCalculationRow: {
    marginTop: 12, minHeight: 68, borderRadius: 7, borderWidth: 1, borderColor: '#223239',
    backgroundColor: '#071115', flexDirection: 'row', alignItems: 'center',
  },
  candlingCalculationItem: { flex: 1, alignItems: 'center', justifyContent: 'center' },
  candlingCalculationDivider: { width: 1, height: 36, backgroundColor: '#26363c' },
  candlingCalculationLabel: { color: '#7c898e', fontSize: 9 },
  candlingCalculationValue: { marginTop: 5, color: '#e8eced', fontSize: 16, fontWeight: '800' },
  candlingCalculationAccent: { color: ORANGE },
  candlingModalActions: { marginTop: 16, flexDirection: 'row', gap: 8 },
  candlingCancelButton: {
    flex: 1, height: 44, borderRadius: 7, borderWidth: 1, borderColor: '#304047',
    alignItems: 'center', justifyContent: 'center',
  },
  candlingCancelText: { color: '#aeb8bb', fontSize: 11, fontWeight: '700' },
  candlingSaveButton: {
    flex: 1.4, height: 44, borderRadius: 7, backgroundColor: ORANGE,
    flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6,
  },
  candlingSaveText: { color: '#ffffff', fontSize: 11, fontWeight: '800' },
